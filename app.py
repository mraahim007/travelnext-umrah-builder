...
if "common_adults" not in st.session_state: st.session_state.common_adults = 1
...
if "c_air" not in st.session_state: st.session_state.c_air = "-- Select Airline --"
if "c_land" not in st.session_state: st.session_state.c_land = "-- Select Airport --"

if "va_curr" not in st.session_state: st.session_state.va_curr = "PKR"
if "va_orig_pp_pkr" not in st.session_state: st.session_state.va_orig_pp_pkr = ""
if "va_sell_pp_pkr" not in st.session_state: st.session_state.va_sell_pp_pkr = ""
if "va_orig_pp_sar" not in st.session_state: st.session_state.va_orig_pp_sar = ""
if "va_sell_sar_pp" not in st.session_state: st.session_state.va_sell_sar_pp = ""
...
if "z_makkah_trans" not in st.session_state: st.session_state.z_makkah_trans = "-- Select Transport --"
if "z_makkah_curr" not in st.session_state: st.session_state.z_makkah_curr = "PKR"
if "z_makkah_orig_pkr" not in st.session_state: st.session_state.z_makkah_orig_pkr = ""
if "z_makkah_sell_pkr" not in st.session_state: st.session_state.z_makkah_sell_pkr = ""
if "z_makkah_orig_sar" not in st.session_state: st.session_state.z_makkah_orig_sar = ""
if "z_makkah_sell_sar" not in st.session_state: st.session_state.z_makkah_sell_sar = ""

if "z_madinah_date" not in st.session_state: st.session_state.z_madinah_date = date.today()
if "z_madinah_trans" not in st.session_state: st.session_state.z_madinah_trans = "-- Select Transport --"
if "z_madinah_curr" not in st.session_state: st.session_state.z_madinah_curr = "PKR"
if "z_madinah_orig_pkr" not in st.session_state: st.session_state.z_madinah_orig_pkr = ""
if "z_madinah_sell_pkr" not in st.session_state: st.session_state.z_madinah_sell_pkr = ""
if "z_madinah_orig_sar" not in st.session_state: st.session_state.z_madinah_orig_sar = ""
if "z_madinah_sell_sar" not in st.session_state: st.session_state.z_madinah_sell_sar = ""

# Initialize legs data up to 10 safely
for i in range(10):
    if f"leg_city_{i}" not in st.session_state: st.session_state[f"leg_city_{i}"] = "-- Select City / Stop --"
    if f"leg_nights_{i}" not in st.session_state: st.session_state[f"leg_nights_{i}"] = 1
    if f"leg_cat_{i}" not in st.session_state: st.session_state[f"leg_cat_{i}"] = "-- Select Category --"
    if f"leg_hotel_{i}" not in st.session_state: st.session_state[f"leg_hotel_{i}"] = "-- Select Specific Hotel --"
    ...
    if f"leg_orig_pn_pkr_{i}" not in st.session_state: st.session_state[f"leg_orig_pn_pkr_{i}"] = ""
    if f"leg_sell_pn_pkr_{i}" not in st.session_state: st.session_state[f"leg_sell_pn_pkr_{i}"] = ""
    if f"leg_orig_pn_sar_{i}" not in st.session_state: st.session_state[f"leg_orig_pn_sar_{i}"] = ""
    if f"leg_sell_pn_sar_{i}" not in st.session_state: st.session_state[f"leg_sell_pn_sar_{i}"] = ""
    if f"leg_trans_veh_{i}" not in st.session_state: st.session_state[f"leg_trans_veh_{i}"] = "-- Select Transport --"
    if f"leg_t_curr_{i}" not in st.session_state: st.session_state[f"leg_t_curr_{i}"] = "PKR"
    if f"leg_t_orig_pkr_{i}" not in st.session_state: st.session_state[f"leg_t_orig_pkr_{i}"] = ""
    if f"leg_t_sell_pkr_{i}" not in st.session_state: st.session_state[f"leg_t_sell_pkr_{i}"] = ""
    if f"leg_t_orig_sar_{i}" not in st.session_state: st.session_state[f"leg_t_orig_sar_{i}"] = ""
    if f"leg_t_sell_sar_{i}" not in st.session_state: st.session_state[f"leg_t_sell_sar_{i}"] = ""
    ...
    for i in range(num_legs):
        ...
        with col_s3:
            if not is_airport_stop and city_val != "-- Select City / Stop --":
                ...
                if curr_h == "PKR":
                    st.text_input(f"Original Cost Per Night {i+1} (PKR):", key=f"leg_orig_pn_pkr_{i}")
                    st.text_input(f"Selling Price Per Night {i+1} (PKR):", key=f"leg_sell_pn_pkr_{i}")
                else:
                    st.text_input(f"Original Cost Per Night {i+1} (SAR):", key=f"leg_orig_pn_sar_{i}")
                    st.text_input(f"Selling Price Per Night {i+1} (SAR):", key=f"leg_sell_pn_sar_{i}")
        ...
        with col_t3:
            ...
            if t_curr_val == "PKR":
                st.text_input(f"Transport Org Cost {i+1} (PKR):", key=f"leg_t_orig_pkr_{i}")
                st.text_input(f"Transport Sell Cost {i+1} (PKR):", key=f"leg_t_sell_pkr_{i}")
            else:
                st.text_input(f"Transport Org Cost {i+1} (SAR):", key=f"leg_t_orig_sar_{i}")
                st.text_input(f"Transport Sell Cost {i+1} (SAR):", key=f"leg_t_sell_sar_{i}")
        ...
        if st.session_state.z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            with c_m3:
                if st.session_state.z_makkah_curr == "PKR":
                    st.text_input("Makkah Ziarats Original Cost (PKR):", key="z_makkah_orig_pkr")
                    st.text_input("Makkah Ziarats Selling Price (PKR):", key="z_makkah_sell_pkr")
                else:
                    st.text_input("Makkah Ziarats Original Cost (SAR):", key="z_makkah_orig_sar")
                    st.text_input("Makkah Ziarats Selling Price (SAR):", key="z_makkah_sell_sar")
        ...
        if st.session_state.z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            with c_md3:
                if st.session_state.z_madinah_curr == "PKR":
                    st.text_input("Madinah Ziarats Original Cost (PKR):", key="z_madinah_orig_pkr")
                    st.text_input("Madinah Ziarats Selling Price (PKR):", key="z_madinah_sell_pkr")
                else:
                    st.text_input("Madinah Ziarats Original Cost (SAR):", key="z_madinah_orig_sar")
                    st.text_input("Madinah Ziarats Selling Price (SAR):", key="z_madinah_sell_sar")
...
# ================= STEP 7: VISA & AIRFARE =================
    ...
    with col18:
        if st.session_state.va_curr == "PKR":
            st.text_input("Original Cost Per Person (PKR):", key="va_orig_pp_pkr")
        else:
            st.text_input("Original Cost Per Person (SAR):", key="va_orig_pp_sar")
    with col19:
        if st.session_state.va_curr == "PKR":
            st.text_input("Selling Price Per Person (PKR):", key="va_sell_pp_pkr")
        else:
            st.text_input("Selling Price Per Person (SAR):", key="va_sell_sar_pp")
    st.markdown("</div>", unsafe_allow_html=True)
...
# ================= STEP 8: FINAL PROFESSIONAL QUOTATION PREVIEW =================
    ...
    for i in range(num_legs):
        ...
        if not is_apt:
            ...
            if curr_h == 'PKR':
                h_sell_pn = parse_val(st.session_state.get(f'leg_sell_pn_pkr_{i}', '0'))
            else:
                h_sell_pn = parse_val(st.session_state.get(f'leg_sell_pn_sar_{i}', '0')) * sar_to_pkr
        ...
        t_sell = 0.0
        if t_curr == 'PKR':
            t_sell = parse_val(st.session_state.get(f'leg_t_sell_pkr_{i}', '0'))
        else:
            t_sell = parse_val(st.session_state.get(f'leg_t_sell_sar_{i}', '0')) * sar_to_pkr
    ...
    if st.session_state.z_status == 'YES':
        ...
        if z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            m_sell = parse_val(st.session_state.z_makkah_sell_pkr) if m_curr == 'PKR' else parse_val(st.session_state.z_makkah_sell_sar) * sar_to_pkr
            ziarat_sell += m_sell
            m_date = st.session_state.z_makkah_date
            m_trans = st.session_state.z_makkah_trans
            ziarats_rows_html += f"""
            <tr style="border-bottom: 1px solid #e2e8f0;">
        ...
        if z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            md_sell = parse_val(st.session_state.z_madinah_sell_pkr) if md_curr == 'PKR' else parse_val(st.session_state.z_madinah_sell_sar) * sar_to_pkr
            ziarat_sell += md_sell
            md_date = st.session_state.z_madinah_date
            md_trans = st.session_state.z_madinah_trans
            ziarats_rows_html += f"""
            <tr style="border-bottom: 1px solid #e2e8f0;">
    ...
    if va_curr == 'PKR':
        va_sell_pp = parse_val(st.session_state.va_sell_pp_pkr)
    else:
        va_sell_pp = parse_val(st.session_state.va_sell_sar_pp) * sar_to_pkr
