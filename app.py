...
if sell_1 > 0:
    sar_preview_1 = sell_1 / sar_to_pkr
    st.caption(f"💱 Leg 1 Selling in SAR: **SAR {sar_preview_1:,.2f}**")
...
if trans_sell_1 > 0:
    t_sar_1 = trans_sell_1 / sar_to_pkr
    st.caption(f"💱 Transport 1 Selling in SAR: **SAR {t_sar_1:,.2f}**")
...
if (city_1 != "-- Select City --") and (is_jeddah_1 or nights_1 > 0):
    ...
    if sell_2 > 0:
        sar_preview_2 = sell_2 / sar_to_pkr
        st.caption(f"💱 Leg 2 Selling in SAR: **SAR {sar_preview_2:,.2f}**")
    ...
    if trans_sell_2 > 0:
        t_sar_2 = trans_sell_2 / sar_to_pkr
        st.caption(f"💱 Transport 2 Selling in SAR: **SAR {t_sar_2:,.2f}**")
...
if (city_2 != "-- Select City --") and (("Jeddah" in city_2) or nights_2 > 0):
    ...
    if sell_3 > 0:
        sar_preview_3 = sell_3 / sar_to_pkr
        st.caption(f"💱 Leg 3 Selling in SAR: **SAR {sar_preview_3:,.2f}**")
    ...
    if city_3 != "-- Select City --":
        ...
        if trans_sell_3 > 0:
            t_sar_3 = trans_sell_3 / sar_to_pkr
            st.caption(f"💱 Transport 3 Selling in SAR: **SAR {t_sar_3:,.2f}**")
...
if ziarat_sell_cost > 0:
    st.caption(f"💱 Ziarats Selling in SAR: **SAR {ziarat_sar:,.2f}**")

st.write("---")

# 6. VISA & AIRFARE
st.subheader("🎫 Step 6: Visa + Airfare (Original vs Selling)")
...
if visa_air_sell > 0:
    st.caption(f"💱 Visa & Airfare Selling in SAR: **SAR {visa_sar:,.2f}**")
...
if st.button("Generate Shareable Quotation 📄", type="primary", key="btn_gen_quotation"):
    ...
    **Trip Start Date:** {trip_start_date.strftime('%d %b %Y')} | **Total Duration:** {total_stay_nights} Nights | **Airline:** {cust_airline if cust_airline != '-- Select Airline --' else 'N/A'}
    ...
    - **Stay Leg 1 ({city_1}):** {'Jeddah Transit / Stop' if 'Jeddah' in city_1 else hotel_1} 
      - *Duration:* {nights_1} Nights ({d1.strftime('%d %b')} to {d2.strftime('%d %b %Y')}) | *Transport:* {trans_1}
    """)
    
    if city_2 != "-- Select City --":
        is_j2 = "Jeddah" in city_2
        st.markdown(f"""
    - **Stay Leg 2 ({city_2}):** {'Jeddah Transit / Stop' if is_j2 else hotel_2} 
      - *Duration:* {nights_2} Nights ({d2.strftime('%d %b')} to {d3.strftime('%d %b %Y')}) | *Transport:* {trans_2}
        """)
    ...
    if city_3 != "-- Select City --":
        ...
        st.markdown(f"""
    - **Stay Leg 3 ({city_3}):** {'Jeddah Transit / Stop' if is_j3 else hotel_3} 
      - *Duration:* {nights_3} Nights ({d3.strftime('%d %b')} to {d4.strftime('%d %b %Y')}) | *Transport:* {trans_3}
        """)
    ...
    ---
    ### 💰 TOTAL PACKAGE PRICE: PKR {grand_total_selling:,.2f}  *(~SAR {grand_total_sar:,.2f})*
...
if st.button("Show Internal Profit Sheet 📈", key="btn_profit_sheet"):
    ...
    - **Hotels Stays Total Profit:** PKR {hotels_total_profit:,.2f} *(Sell: {hotels_sell_total:,.2f} / ~SAR {hotels_sar_total:,.2f})*
    - **City Transports Total Profit:** PKR {total_transport_profit:,.2f} *(Sell: {total_transport_sell:,.2f} / ~SAR {transport_sar_total:,.2f})*
    - **Ziarats Profit:** PKR {ziarat_profit:,.2f} *(Sell: {ziarat_sell_cost:,.2f} / ~SAR {ziarat_sar:,.2f})*
    - **Visa & Airfare Profit:** PKR {visa_profit:,.2f} *(Sell: {visa_air_sell:,.2f} / ~SAR {visa_sar:,.2f})*
