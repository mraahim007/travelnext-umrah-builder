...
# ================= STEP 5: HOTEL STAYS & TRANSPORTS =================
    ...
    if (city_1 != "-- Select City / Stop --") and (is_airport_stop_1 or nights_1 > 0):
        ...
        # Stay Leg 3 (Optional)
        if (city_2 != "-- Select City / Stop --") and (is_airport_stop_2 or nights_2 > 0):
            st.markdown("---")
            st.markdown("### 🧳 Stay Leg 3 (Final / Third Stop)")
            ...
            with col_s8:
                if not is_airport_stop_3 and city_3 != "-- Select City / Stop --":
            ...
            with col_s9:
                if not is_airport_stop_3 and city_3 != "-- Select City / Stop --":
                    ...
                    if curr_type_3 == "PKR":
                        orig_pn_3 = parse_val(st.text_input("Original Cost Per Night 3 (PKR):", value="", key="c3_orig_pn_pkr"))
                        sell_pn_3 = parse_val(st.text_input("Selling Price Per Night 3 (PKR):", value="", key="c3_sell_pn_pkr"))
                    else:
                        orig_pn_sar_3 = parse_val(st.text_input("Original Cost Per Night 3 (SAR):", value="", key="c3_orig_pn_sar"))
                        sell_pn_sar_3 = parse_val(st.text_input("Selling Price Per Night 3 (SAR):", value="", key="c3_sell_pn_sar"))
            ...
            with col_t9:
                if t_curr_3 == "PKR":
                    trans_orig_3 = parse_val(st.text_input("Transport Org Cost 3 (PKR):", value="", key="t3_orig_pkr"))
                    trans_sell_3 = parse_val(st.text_input("Transport Sell Cost 3 (PKR):", value="", key="t3_sell_pkr"))
                else:
                    t_orig_sar_3 = parse_val(st.text_input("Transport Org Cost 3 (SAR):", value="", key="t3_orig_sar"))
                    t_sell_sar_3 = parse_val(st.text_input("Transport Sell Cost 3 (SAR):", value="", key="t3_sell_sar"))
...
# ================= STEP 8: FINAL QUOTATION PREVIEW =================
    ...
    c3_n = st.session_state.get('c3_nights', 0)
    c3_r = st.session_state.get('c3_num_rooms', 1)
    c3_sell_pn = st.session_state.get('c3_sell_pn_pkr', 0.0) if st.session_state.get('curr_type_3', 'PKR') == 'PKR' else st.session_state.get('c3_sell_pn_sar', 0.0) * sar_to_pkr
    hotel_total_3 = c3_sell_pn * c3_n * c3_r if c3_n > 0 else 0.0
    
    total_hotels_selling = hotel_total_1 + hotel_total_2 + hotel_total_3
    ...
    t_sell_3 = st.session_state.get('t3_sell_pkr', 0.0) if st.session_state.get('t_curr_3', 'PKR') == 'PKR' else st.session_state.get('t3_sell_sar', 0.0) * sar_to_pkr
    total_transport_selling = t_sell_1 + t_sell_2 + t_sell_3
