...
if "c_full_name" not in st.session_state: st.session_state.c_full_name = ""
...
if "return_air" not in st.session_state: st.session_state.return_air = "Saudia Airlines"
if "return_date" not in st.session_state: st.session_state.return_date = date.today() + timedelta(days=7)
if "return_airport" not in st.session_state: st.session_state.return_airport = "Jeddah Airport (JED)"

if "va_curr" not in st.session_state: st.session_state.va_curr = "PKR"
if "va_cost_val" not in st.session_state: st.session_state.va_cost_val = 0.0
if "va_sell_val" not in st.session_state: st.session_state.va_sell_val = 0.0

if "z_status" not in st.session_state: st.session_state.z_status = "-- Select Option --"
if "z_scope" not in st.session_state: st.session_state.z_scope = "-- Select Scope --"
...
if "z_makkah_cost_val" not in st.session_state: st.session_state.z_makkah_cost_val = 0.0
if "z_makkah_sell_val" not in st.session_state: st.session_state.z_makkah_sell_val = 0.0
...
if "z_madinah_cost_val" not in st.session_state: st.session_state.z_madinah_cost_val = 0.0
if "z_madinah_sell_val" not in st.session_state: st.session_state.z_madinah_sell_val = 0.0
...
for i in range(10):
    ...
    if f"leg_cost_val_{i}" not in st.session_state: st.session_state[f"leg_cost_val_{i}"] = 0.0
    if f"leg_sell_val_{i}" not in st.session_state: st.session_state[f"leg_sell_val_{i}"] = 0.0
    ...
    if f"leg_t_cost_val_{i}" not in st.session_state: st.session_state[f"leg_t_cost_val_{i}"] = 0.0
    if f"leg_t_sell_val_{i}" not in st.session_state: st.session_state[f"leg_t_sell_val_{i}"] = 0.0
...
# ================= STEP 2: CURRENCY EXCHANGE =================
    ...
    st.number_input("Enter current 1 SAR to PKR rate today:", min_value=0.1, max_value=100.0, step=0.5, key="daily_sar_rate", format="%.2f")
...
# ================= STEP 3: CUSTOMER DETAILS =================
    ...
    st.text_input("Customer Full Name *:", key="c_full_name", placeholder="e.g. Muhammad Ali")
    
    col1, col2 = st.columns(2)
    with col1:
        st.number_input("Enter number of adults:", min_value=1, step=1, key="common_adults")
    with col2:
        st.number_input("Enter number of children:", min_value=0, step=1, key="common_children")
    
    t_start = st.date_input("Overall Trip Start Date (Arrival Date):", key="trip_start")
    st.caption(f"📅 Selected Arrival Day: **{t_start.strftime('%A')}**")
...
# ================= STEP 4: FLIGHT DETAILS =================
    ...
    st.markdown("<div class='step-box'>", unsafe_allow_html=True)
    st.subheader("✈️ Step 4: Arrival Flight Information")
    col3, col4 = st.columns(2)
    with col3:
        st.selectbox("Select Arrival Airline Name *:", ["-- Select Airline --", "Saudia Airlines", "PIA", "AirBlue", "SereneAir", "Fly Jinnah", "Emirates", "Qatar Airways"], key="c_air")
    with col4:
...
# ================= STEP 5: HOTEL STAYS & UNLIMITED DYNAMIC LEGS =================
    ...
    for i in range(num_legs):
        ...
        with col_s1:
            ...
            stay_end_date = current_date_cursor + timedelta(days=max(1, nights_val)) if not is_airport_stop else current_date_cursor
            st.caption(f"📅 Stay: {current_date_cursor.strftime('%d %b %Y (%A)')} to {stay_end_date.strftime('%d %b %Y (%A)')}")
            if not is_airport_stop:
                current_date_cursor = stay_end_date
        ...
        with col_s3:
            if not is_airport_stop and city_val != "-- Select City / Stop --":
                ...
                st.number_input(f"Actual Cost Per Night {i+1} ({curr_h}):", min_value=0.0, step=100.0, format="%.2f", key=f"leg_cost_val_{i}")
                st.number_input(f"Selling Price Per Night {i+1} ({curr_h}):", min_value=0.0, step=100.0, format="%.2f", key=f"leg_sell_val_{i}")
            else:
                st.info("ℹ️ No hotel cost for transit.")
        ...
        with col_t3:
            ...
            st.number_input(f"Transport Actual Cost {i+1} ({t_curr_val}):", min_value=0.0, step=100.0, format="%.2f", key=f"leg_t_cost_val_{i}")
            st.number_input(f"Transport Sell Cost {i+1} ({t_curr_val}):", min_value=0.0, step=100.0, format="%.2f", key=f"leg_t_sell_val_{i}")
...
# ================= STEP 6: ZIARATS MANAGEMENT =================
    ...
    if st.session_state.z_status == "YES":
        ...
        if st.session_state.z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            with c_m1:
                m_date = st.date_input("📅 Makkah Ziarat Date:", key="z_makkah_date")
                st.caption(f"Day: **{m_date.strftime('%A')}**")
            ...
            with c_m3:
                ...
                st.number_input(f"Makkah Ziarats Actual Cost ({m_curr}):", min_value=0.0, step=100.0, format="%.2f", key="z_makkah_cost_val")
                st.number_input(f"Makkah Ziarats Selling Price ({m_curr}):", min_value=0.0, step=100.0, format="%.2f", key="z_makkah_sell_val")
        ...
        if st.session_state.z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            with c_md1:
                md_date = st.date_input("📅 Madinah Ziarat Date:", key="z_madinah_date")
                st.caption(f"Day: **{md_date.strftime('%A')}**")
            ...
            with c_md3:
                ...
                st.number_input(f"Madinah Ziarats Actual Cost ({md_curr}):", min_value=0.0, step=100.0, format="%.2f", key="z_madinah_cost_val")
                st.number_input(f"Madinah Ziarats Selling Price ({md_curr}):", min_value=0.0, step=100.0, format="%.2f", key="z_madinah_sell_val")
...
# ================= STEP 7: VISA & AIRFARE =================
    ...
    with col18:
        ...
        st.number_input(f"Actual Cost Per Person ({va_curr}):", min_value=0.0, step=500.0, format="%.2f", key="va_cost_val")
        st.number_input(f"Selling Price Per Person ({va_curr}):", min_value=0.0, step=500.0, format="%.2f", key="va_sell_val")
    st.markdown("</div>", unsafe_allow_html=True)

# ================= STEP 8: RETURN FLIGHT DETAILS =================
elif current_step == 8:
    st.markdown("<div class='step-box'>", unsafe_allow_html=True)
    st.subheader("🛫 Step 8: Return Flight Information")
    col_rf1, col_rf2 = st.columns(2)
    with col_rf1:
        st.selectbox("Select Return Airline *:", ["-- Select Airline --", "Saudia Airlines", "PIA", "AirBlue", "SereneAir", "Fly Jinnah", "Emirates", "Qatar Airways"], key="return_air")
        ret_d = st.date_input("Return Flight Date *:", key="return_date")
        st.caption(f"📅 Return Day: **{ret_d.strftime('%A')}**")
    with col_rf2:
        st.selectbox("Return Departure Airport *:", ["-- Select Airport --", "Jeddah Airport (JED)", "Madinah Airport (MED)"], key="return_airport")
    st.markdown("</div>", unsafe_allow_html=True)

# ================= STEP 9: FINAL PROFESSIONAL QUOTATION PREVIEW =================
elif current_step == 9:
    st.markdown("<div class='step-box'>", unsafe_allow_html=True)
    st.subheader("📄 Step 9: Official Customer Quotation & Actions")
    
    full_client_name = st.session_state.get("c_full_name", "").strip()
    if not full_client_name:
        full_client_name = "Valued Customer"
    
    adults_cnt = st.session_state.get("common_adults", 2)
    ...
    airline_name = st.session_state.get("c_air", "Saudia Airlines")
    landing_port = st.session_state.get("c_land", "Jeddah Airport (JED)")
    
    ret_airline = st.session_state.get("return_air", "Saudia Airlines")
    ret_date = st.session_state.get("return_date", date.today() + timedelta(days=7))
    ret_airport = st.session_state.get("return_airport", "Jeddah Airport (JED)")
...
══════════════════════
👤 *Client Name:* {full_client_name}
📅 *Arrival Date:* {t_date.strftime('%d %b %Y (%A)')}
👥 *Total Travelers:* {tot_people} Persons ({adults_cnt} Adults, {child_cnt} Children)
✈️ *Arrival Flight:* {airline_name if airline_name != '-- Select Airline --' else 'N/A'} ({landing_port})
🛫 *Return Flight:* {ret_airline if ret_airline != '-- Select Airline --' else 'N/A'} on {ret_date.strftime('%d %b %Y (%A)')} from {ret_airport}
──────────────────────
🏨 *Itinerary & Services:*"""
    ...
    for i in range(num_legs):
        ...
        if not is_apt:
            ...
            raw_h_sell = float(st.session_state.get(f'leg_sell_val_{i}') or 0.0)
            h_sell_pn = raw_h_sell if curr_h == 'PKR' else raw_h_sell * sar_to_pkr
        ...
        raw_t_sell = float(st.session_state.get(f'leg_t_sell_val_{i}') or 0.0)
        t_sell = raw_t_sell if t_curr == 'PKR' else raw_t_sell * sar_to_pkr
        ...
        if not is_apt:
            itinerary_html_rows += f"<tr><td style='padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;'><b>Leg {i+1}</b></td><td style='padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;'><b>{c_city}</b><br><small>{stay_from.strftime('%d %b')} - {stay_to.strftime('%d %b %Y')}</small></td><td style='padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top; line-height: 1.5;'><b>Hotel:</b> {c_hotel}<br><b>Stay:</b> {c_nights} Nights, {c_rooms} Room(s) | <b>Bed/View:</b> {c_bed} | <b>Meal:</b> {c_meal}<br>🚗 <i>Transfer:</i> {t_route_desc}</td></tr>"
            share_text += f"\n\n▫️ *Leg {i+1} ({c_city}):*\n   • Hotel: {c_hotel}\n   • Stay: {c_nights} Nights | {c_rooms} Room(s)\n   • Bed/View: {c_bed} | Meal: {c_meal}\n   • Transfer: {t_route_desc}"
        else:
            itinerary_html_rows += f"<tr><td style='padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;'><b>Leg {i+1}</b></td><td style='padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;'><b>{c_city}</b></td><td style='padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top; line-height: 1.5;'>Airport Transit / Stop<br>🚗 <i>Transfer:</i> {t_route_desc}</td></tr>"
            share_text += f"\n\n▫️ *Leg {i+1} ({c_city}):*\n   • Airport Transit / Stop\n   • Transfer: {t_route_desc}"

    ziarat_sell = 0.0
    ziarat_text = "Not Included"
    if st.session_state.z_status == 'YES':
        z_scope = st.session_state.get('z_scope', '-- Select Scope --')
        ziarat_text_parts = []
        
        if z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            m_curr = st.session_state.z_makkah_curr
            raw_m_sell = float(st.session_state.z_makkah_sell_val or 0.0)
            m_sell = raw_m_sell if m_curr == 'PKR' else raw_m_sell * sar_to_pkr
            ziarat_sell += m_sell
            m_dt = st.session_state.z_makkah_date
            ziarat_text_parts.append(f"Makkah Ziarat ({m_dt.strftime('%d %b, %A')})")

        if z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            md_curr = st.session_state.z_madinah_curr
            raw_md_sell = float(st.session_state.z_madinah_sell_val or 0.0)
            md_sell = raw_md_sell if md_curr == 'PKR' else raw_md_sell * sar_to_pkr
            ziarat_sell += md_sell
            md_dt = st.session_state.z_madinah_date
            ziarat_text_parts.append(f"Madinah Ziarat ({md_dt.strftime('%d %b, %A')})")
    ...
    va_curr = st.session_state.va_curr
    raw_va_sell = float(st.session_state.va_sell_val or 0.0)
    va_sell_pp = raw_va_sell if va_curr == 'PKR' else raw_va_sell * sar_to_pkr
...
_Thanks for choosing TravelNext. Aao Madiney Chalen!_"""
    ...
    <div class="invoice-box">
        <div class="header-section">
            <h2 style="color: #2e8b57; margin: 0; font-size: 22px; letter-spacing: 0.5px;">TRAVELNEXT INTERNATIONAL</h2>
            <p style="margin: 6px 0 0 0; font-size: 13.5px; color: #555; font-weight: 500;">Official Umrah Packages & Travel Services | Aao Madiney Chalen</p>
        </div>
        
        <table class="details-table">
            <tr>
                <td><b>Client Name:</b> {full_client_name}</td>
                <td><b>Arrival Date:</b> {t_date.strftime('%d %b %Y (%A)')}</td>
            </tr>
            <tr>
                <td><b>Total Travelers:</b> {tot_people} Persons ({adults_cnt} Adults, {child_cnt} Children)</td>
                <td><b>Arrival Flight:</b> {airline_name if airline_name != '-- Select Airline --' else 'N/A'} ({landing_port})</td>
            </tr>
            <tr>
                <td><b>Return Flight:</b> {ret_airline if ret_airline != '-- Select Airline --' else 'N/A'}</td>
                <td><b>Return Date & Airport:</b> {ret_date.strftime('%d %b %Y (%A)')} from {ret_airport}</td>
            </tr>
        </table>
        
        <h4 style="color: #2e8b57; border-bottom: 1px solid #e0e0e0; padding-bottom: 6px; margin-bottom: 10px; font-size: 15px;">Hotel Itinerary & Transport Breakdown</h4>
        <table class="itinerary-table">
            ...
            {itinerary_html_rows}
            <tr>
                <td style="padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;"><b>Ziarats</b></td>
                <td style="padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;" colspan="2">{ziarat_text}</td>
            </tr>
            <tr>
                <td style="padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;"><b>Visa & Air</b></td>
                <td style="padding: 10px 12px; border: 1px solid #d0e8dc; vertical-align: top;" colspan="2">PKR {total_visa_air:,.2f} total ({tot_people} travelers @ PKR {va_sell_pp:,.2f} per person)</td>
            </tr>
        </table>
        
        <div class="total-box">
            <h3 style="margin: 0; color: #1b5e3a; font-size: 18.5px;">TOTAL PACKAGE INVESTMENT: PKR {grand_total_selling:,.2f}</h3>
            <p style="margin: 6px 0 0 0; font-size: 14px; color: #444; font-weight: 500;">Equivalent to approx. SAR {grand_total_sar:,.2f}</p>
        </div>
        
        <p style="text-align: center; font-size: 12.5px; color: #666; margin-top: 10px; font-style: italic;">Thanks for choosing TravelNext. May Allah accept your good deeds!</p>
...
# ================= STEP 10: AGENCY PROFIT DASHBOARD =================
elif current_step == 10:
    st.markdown("<div class='step-box'>", unsafe_allow_html=True)
    st.subheader("📊 Step 10: Internal Agency Profit & Margin Report")
    ...
    for i in range(st.session_state.num_legs):
        ...
        raw_c_cost = float(st.session_state.get(f'leg_cost_val_{i}') or 0.0)
        raw_c_sell = float(st.session_state.get(f'leg_sell_val_{i}') or 0.0)
        
        c_cost = raw_c_cost if curr_h == 'PKR' else raw_c_cost * sar_to_pkr
        c_sell = raw_c_sell if curr_h == 'PKR' else raw_c_sell * sar_to_pkr
            
        tot_cost_hotels += c_cost * c_nights * c_rooms
        tot_sell_hotels += c_sell * c_nights * c_rooms
        
        t_curr = st.session_state.get(f'leg_t_curr_{i}', 'PKR')
        raw_t_cost = float(st.session_state.get(f'leg_t_cost_val_{i}') or 0.0)
        raw_t_sell = float(st.session_state.get(f'leg_t_sell_val_{i}') or 0.0)
        
        t_cost = raw_t_cost if t_curr == 'PKR' else raw_t_cost * sar_to_pkr
        t_sell = raw_t_sell if t_curr == 'PKR' else raw_t_sell * sar_to_pkr
