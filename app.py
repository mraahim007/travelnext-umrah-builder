...
if "c_first_name" not in st.session_state: st.session_state.c_first_name = "Valued"
if "c_last_name" not in st.session_state: st.session_state.c_last_name = "Customer"
if "c_passport" not in st.session_state: st.session_state.c_passport = "PK-9876543"
...
if "va_sell_pp_pkr" not in st.session_state: st.session_state.va_sell_pp_pkr = 45000.0
...
if "z_makkah_sell_pkr" not in st.session_state: st.session_state.z_makkah_sell_pkr = 15000.0
...
if "z_madinah_sell_pkr" not in st.session_state: st.session_state.z_madinah_sell_pkr = 15000.0

# Initialize legs data up to 10 safely with intelligent default values
...
for i in range(10):
    ...
    if f"leg_sell_pn_pkr_{i}" not in st.session_state: st.session_state[f"leg_sell_pn_pkr_{i}"] = 35000.0
    ...
    if f"leg_t_sell_pkr_{i}" not in st.session_state: st.session_state[f"leg_t_sell_pkr_{i}"] = 20000.0
...
# ================= STEP 5: HOTEL STAYS & UNLIMITED DYNAMIC LEGS =================
    ...
    st.markdown("Configure your stay legs and dynamic transportation routes seamlessly.")
...
# ================= STEP 7: VISA & AIRFARE =================
    ...
    with col18:
        if st.session_state.va_curr == "PKR":
            ...
            st.number_input("Selling Price Per Person (SAR):", min_value=0.0, step=100.0, format="%.2f", key="va_sell_sar_pp")
    st.markdown("</div>", unsafe_allow_html=True)
...
# ================= STEP 8: FINAL PROFESSIONAL QUOTATION PREVIEW =================
    ...
    if st.session_state.z_status == 'YES':
        ...
        if z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            m_sell = float(st.session_state.get('z_makkah_sell_pkr', 0.0)) if m_curr == 'PKR' else float(st.session_state.get('z_makkah_sell_sar', 0.0)) * sar_to_pkr
            ziarat_sell += m_sell
        ...
        if z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            md_sell = float(st.session_state.get('z_makkah_sell_pkr', 0.0)) if md_curr == 'PKR' else float(st.session_state.get('z_makkah_sell_sar', 0.0)) * sar_to_pkr
            ziarat_sell += md_sell
    ...
    if va_curr == 'PKR':
        ...
        va_sell_pp = float(st.session_state.get('va_sell_sar_pp', 0.0)) * sar_to_pkr
