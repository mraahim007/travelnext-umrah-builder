    ...
    for i in range(num_legs):
        ...
        with col_s3:
            if not is_airport_stop and city_val != "-- Select City / Stop --":
                ...
                if curr_h == "PKR":
                    st.text_input(f"Original Cost Per Night {i+1} (PKR):", value="0", key=f"leg_orig_pn_pkr_{i}")
                    st.text_input(f"Selling Price Per Night {i+1} (PKR):", value="0", key=f"leg_sell_pn_pkr_{i}")
                else:
                    st.text_input(f"Original Cost Per Night {i+1} (SAR):", value="0", key=f"leg_orig_pn_sar_{i}")
                    st.text_input(f"Selling Price Per Night {i+1} (SAR):", value="0", key=f"leg_sell_pn_sar_{i}")
        ...
        with col_t3:
            ...
            if t_curr_val == "PKR":
                st.text_input(f"Transport Org Cost {i+1} (PKR):", value="0", key=f"leg_t_orig_pkr_{i}")
                st.text_input(f"Transport Sell Cost {i+1} (PKR):", value="0", key=f"leg_t_sell_pkr_{i}")
            else:
                st.text_input(f"Transport Org Cost {i+1} (SAR):", value="0", key=f"leg_t_orig_sar_{i}")
                st.text_input(f"Transport Sell Cost {i+1} (SAR):", value="0", key=f"leg_t_sell_sar_{i}")
        ...
        if ziarat_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            with c_m3:
                if st.session_state.get("z_makkah_curr", "PKR") == "PKR":
                    st.text_input("Makkah Ziarats Original Cost (PKR):", value="0", key="z_makkah_orig_pkr")
                    st.text_input("Makkah Ziarats Selling Price (PKR):", value="0", key="z_makkah_sell_pkr")
                else:
                    st.text_input("Makkah Ziarats Original Cost (SAR):", value="0", key="z_makkah_orig_sar")
                    st.text_input("Makkah Ziarats Selling Price (SAR):", value="0", key="z_makkah_sell_sar")
        ...
        if ziarat_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            with c_md3:
                if st.session_state.get("z_madinah_curr", "PKR") == "PKR":
                    st.text_input("Madinah Ziarats Original Cost (PKR):", value="0", key="z_madinah_orig_pkr")
                    st.text_input("Madinah Ziarats Selling Price (PKR):", value="0", key="z_madinah_sell_pkr")
                else:
                    st.text_input("Madinah Ziarats Original Cost (SAR):", value="0", key="z_madinah_orig_sar")
                    st.text_input("Madinah Ziarats Selling Price (SAR):", value="0", key="z_madinah_sell_sar")
...
# ================= STEP 7: VISA & AIRFARE =================
    ...
    with col18:
        if va_curr == "PKR":
            st.text_input("Original Cost Per Person (PKR):", value="0", key="va_orig_pp_pkr")
        else:
            st.text_input("Original Cost Per Person (SAR):", value="0", key="va_orig_pp_sar")
    with col19:
        if va_curr == "PKR":
            st.text_input("Selling Price Per Person (PKR):", value="0", key="va_sell_pp_pkr")
        else:
            st.text_input("Selling Price Per Person (SAR):", value="0", key="va_sell_sar_pp")
    st.markdown("</div>", unsafe_allow_html=True)
...
# ================= STEP 8: FINAL PROFESSIONAL QUOTATION PREVIEW =================
    ...
    for i in range(num_legs):
        ...
        if not is_apt:
            ...
            if curr_h == 'PKR':
                h_sell_pn = parse_val(st.session_state.get(f'leg_sell_pn_pkr_{i}', 0.0))
            else:
                h_sell_pn = parse_val(st.session_state.get(f'leg_sell_pn_sar_{i}', 0.0)) * sar_to_pkr
        ...
        t_sell = 0.0
        if t_curr == 'PKR':
            t_sell = parse_val(st.session_state.get(f'leg_t_sell_pkr_{i}', 0.0))
        else:
            t_sell = parse_val(st.session_state.get(f'leg_t_sell_sar_{i}', 0.0)) * sar_to_pkr
    ...
    if st.session_state.get('z_status') == 'YES':
        ...
        if z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            m_sell = parse_val(st.session_state.get('z_makkah_sell_pkr', 0.0)) if m_curr == 'PKR' else parse_val(st.session_state.get('z_makkah_sell_sar', 0.0)) * sar_to_pkr
            ziarat_sell += m_sell
        ...
        if z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            md_sell = parse_val(st.session_state.get('z_madinah_sell_pkr', 0.0)) if md_curr == 'PKR' else parse_val(st.session_state.get('z_madinah_sell_sar', 0.0)) * sar_to_pkr
            ziarat_sell += md_sell
