...
# 7. OFFICIAL CUSTOMER QUOTATION WITH WHATSAPP SHARE BUTTON
...
if st.button("Generate Quotation 📋", key="btn_gen_quotation"):
    ...
    # Building clear text representation for sharing
...
*🏨 Itinerary:*"""
    ...
    if city_1 != "-- Select City --":
        ...
        if not is_j1:
            st.markdown(f"- **Leg 1 ({city_1}):** {hotel_1}")
            st.markdown(f"  * *Duration:* {nights_1} Nights | *Rooms:* {num_rooms_1} ({bed_type_1}) | *Meal:* {meal_plan_1} | *Transport:* {trans_1}")
            share_text += f"\n- Leg 1 ({city_1}): {hotel_1} ({nights_1} Nights, {num_rooms_1} Room(s), {bed_type_1}, {meal_plan_1})"
        else:
            st.markdown(f"- **Leg 1 ({city_1}):** Jeddah Transit / Stop | *Transport:* {trans_1}")
            share_text += f"\n- Leg 1 ({city_1}): Transit / Stop"

    if city_2 != "-- Select City --":
        is_j2 = "Jeddah" in city_2
        if not is_j2:
            st.markdown(f"- **Leg 2 ({city_2}):** {hotel_2}")
            st.markdown(f"  * *Duration:* {nights_2} Nights | *Rooms:* {num_rooms_2} ({bed_type_2}) | *Meal:* {meal_plan_2} | *Transport:* {trans_2}")
            share_text += f"\n- Leg 2 ({city_2}): {hotel_2} ({nights_2} Nights, {num_rooms_2} Room(s), {bed_type_2}, {meal_plan_2})"
        else:
            st.markdown(f"- **Leg 2 ({city_2}):** Jeddah Transit / Stop | *Transport:* {trans_2}")
            share_text += f"\n- Leg 2 ({city_2}): Transit / Stop"

    if city_3 != "-- Select City --":
        is_j3 = "Jeddah" in city_3
        d4 = d3 + timedelta(days=nights_3)
        if not is_j3:
            st.markdown(f"- **Leg 3 ({city_3}):** {hotel_3}")
            st.markdown(f"  * *Duration:* {nights_3} Nights | *Rooms:* {num_rooms_3} ({bed_type_3}) | *Meal:* {meal_plan_3} | *Transport:* {trans_3}")
            share_text += f"\n- Leg 3 ({city_3}): {hotel_3} ({nights_3} Nights, {num_rooms_3} Room(s), {bed_type_3}, {meal_plan_3})"
        else:
            st.markdown(f"- **Leg 3 ({city_3}):** Jeddah Transit / Stop | *Transport:* {trans_3}")
            share_text += f"\n- Leg 3 ({city_3}): Transit / Stop"

    ziarat_text = f"{ziarat_scope} (Transport: {ziarat_transport_type})" if ziarat_status == "YES" else "Not Included"
    st.markdown(f"#### 🗺️ Ziarats & Services\n- **Ziarats:** {ziarat_text}")
    share_text += f"\n- *Ziarats:* {ziarat_text}"
    ...
    # Share Button
    st.markdown(f'<a href="{whatsapp_url}" target="_blank"><button style="background-color:#25D366; color:white; padding:12px 20px; border:none; border-radius:8px; font-size:16px; font-weight:bold; cursor:pointer;">📤 Share Quotation via WhatsApp</button></a>', unsafe_allow_html=True)
