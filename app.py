...
# 2. CUSTOMER DETAILS & FLIGHT INFO (Updated with Infants & Separate Ticket Inputs)
...
with col1:
    ...
    adults = st.number_input("Number of Adults (above 12):", min_value=1, value=1, key="common_adults")
    children = st.number_input("Number of Children (below 12):", min_value=0, value=0, key="common_children")
    infants = st.number_input("Number of Infants (below 2):", min_value=0, value=0, key="common_infants")
...
with col2:
    ...
    trip_start_date = st.date_input("Trip / Arrival Start Date:", value=date.today(), key="trip_start")
...
total_passengers_for_fare = adults + children  # Infants usually don't occupy full seat or counted separately, but let's keep separate pricing inputs as requested

st.info(f"📅 Arrival Day: **{arrival_day_name}** | Travelers ➔ Adults: **{adults}**, Children: **{children}**, Infants: **{infants}**")

st.markdown("#### ✈️ Arrival Flight Pricing (Per Category)")
arr_curr = st.radio("Currency (Arrival Flight):", ["PKR", "SAR"], horizontal=True, key="arr_curr")

ac1, ac2, ac3 = st.columns(3)
with ac1:
    st.markdown("<b>Adult Fare (Per Person)</b>", unsafe_allow_html=True)
    if arr_curr == "PKR":
        arr_orig_ad = st.number_input("Adult Actual (PKR):", min_value=0.0, value=None, step=1000.0, format="%.2f", key="arr_orig_ad_pkr") or 0.0
        arr_sell_ad = st.number_input("Adult Selling (PKR):", min_value=0.0, value=None, step=1000.0, format="%.2f", key="arr_sell_ad_pkr") or 0.0
    else:
        o_sar = st.number_input("Adult Actual (SAR):", min_value=0.0, value=None, step=100.0, format="%.2f", key="arr_orig_ad_sar") or 0.0
        s_sar = st.number_input("Adult Selling (SAR):", min_value=0.0, value=None, step=100.0, format="%.2f", key="arr_sell_ad_sar") or 0.0
        arr_orig_ad, arr_sell_ad = o_sar * sar_to_pkr, s_sar * sar_to_pkr

with ac2:
    st.markdown("<b>Child Fare (Per Person)</b>", unsafe_allow_html=True)
    if arr_curr == "PKR":
        arr_orig_ch = st.number_input("Child Actual (PKR):", min_value=0.0, value=None, step=1000.0, format="%.2f", key="arr_orig_ch_pkr") or 0.0
        arr_sell_ch = st.number_input("Child Selling (PKR):", min_value=0.0, value=None, step=1000.0, format="%.2f", key="arr_sell_ch_pkr") or 0.0
    else:
        o_sar = st.number_input("Child Actual (SAR):", min_value=0.0, value=None, step=100.0, format="%.2f", key="arr_orig_ch_sar") or 0.0
        s_sar = st.number_input("Child Selling (SAR):", min_value=0.0, value=None, step=100.0, format="%.2f", key="arr_sell_ch_sar") or 0.0
        arr_orig_ch, arr_sell_ch = o_sar * sar_to_pkr, s_sar * sar_to_pkr

with ac3:
    st.markdown("<b>Infant Fare (Per Person)</b>", unsafe_allow_html=True)
    if arr_curr == "PKR":
        arr_orig_inf = st.number_input("Infant Actual (PKR):", min_value=0.0, value=None, step=500.0, format="%.2f", key="arr_orig_inf_pkr") or 0.0
        arr_sell_inf = st.number_input("Infant Selling (PKR):", min_value=0.0, value=None, step=500.0, format="%.2f", key="arr_sell_inf_pkr") or 0.0
    else:
        o_sar = st.number_input("Infant Actual (SAR):", min_value=0.0, value=None, step=50.0, format="%.2f", key="arr_orig_inf_sar") or 0.0
        s_sar = st.number_input("Infant Selling (SAR):", min_value=0.0, value=None, step=50.0, format="%.2f", key="arr_sell_inf_sar") or 0.0
        arr_orig_inf, arr_sell_inf = o_sar * sar_to_pkr, s_sar * sar_to_pkr

arrival_orig_total = (arr_orig_ad * adults) + (arr_orig_ch * children) + (arr_orig_inf * infants)
arrival_sell_total = (arr_sell_ad * adults) + (arr_sell_ch * children) + (arr_sell_inf * infants)

if arrival_sell_total > 0:
    arr_sar_prev = arrival_sell_total / sar_to_pkr if sar_to_pkr > 0 else 0
    st.success(f"✈️ Arrival Flight Total Total ➔ **PKR {arrival_sell_total:,.2f}** | **SAR {arr_sar_prev:,.2f}**")
...
# 3. UNLIMITED DYNAMIC STAY & TRANSPORT BUILDER WITH CALENDARS & AUTO NIGHTS
st.subheader("🏨 Step 3: Hotel Stays & City Transport Builder (With Calendars)")
...
for i in range(st.session_state.num_legs):
    ...
    with col_s1:
        ...
        # Calendar pickers for Leg check-in and check-out dates to auto-calculate nights
        st.markdown(f"<b>Stay Dates (Leg {leg_num}):</b>", unsafe_allow_html=True)
        leg_checkin = st.date_input(f"Check-in Date (Leg {leg_num}):", value=running_date, key=f"checkin_{leg_num}")
        default_checkout = leg_checkin + timedelta(days=3) if not is_airport_stop else leg_checkin
        leg_checkout = st.date_input(f"Check-out Date (Leg {leg_num}):", value=default_checkout, key=f"checkout_{leg_num}")
        
        if is_airport_stop:
            nights = 0
        else:
            delta_days = (leg_checkout - leg_checkin).days
            nights = max(0, delta_days)
            st.info(f"⏳ Auto-calculated Stay: **{nights} Nights** ({leg_checkin.strftime('%d %b')} to {leg_checkout.strftime('%d %b')})")
        
        # Update running date for next leg
        running_date = leg_checkout if not is_airport_stop else leg_checkin
    ...
    # Transport for this leg with Calendar date
    ...
    with col_t1:
        ...
        trans_date = st.date_input(f"Transfer Date ({prev_city_label} ➔ {current_dest_label}):", value=leg_checkin, key=f"t_date_{leg_num}")
    with col_t2:
...
if ziarat_status == "YES":
    ...
    if ziarat_scope in ["MAKKAH ONLY", "MADINAH ONLY"]:
        ...
        with zc1:
            ...
            ziarat_date = st.date_input("Ziarat Date:", value=trip_start_date, key="z_date_single")
        with zc2:
        ...
        with zm1:
            ...
            makkah_z_date = st.date_input("Makkah Ziarat Date:", value=trip_start_date, key="makkah_z_date")
        with zm2:
        ...
        with zmd1:
            ...
            madinah_z_date = st.date_input("Madinah Ziarat Date:", value=trip_start_date, key="madinah_z_date")
        with zmd2:
...
# 5. VISA SERVICES & RETURN AIRPORT FLIGHT (Rates removed from return flight as requested, kept Airline & Airport only)
st.subheader("🎫 Step 5: Visa Services & Return Flight Details")
...
with col_v2:
    st.markdown("#### ✈️ Return Flight Details")
    ...
    st.info("ℹ️ Return flight rates are included or handled separately as per flight routing.")

total_travelers_count = adults + children + infants

visa_orig_total = visa_orig_pp * total_travelers_count
visa_sell_total = visa_sell_pp * total_travelers_count
visa_profit = visa_sell_total - visa_orig_total

# No financial impact from separate return rate input here since it's wrapped in flight building or handled via general package cost
return_orig_total = 0.0
return_sell_total = 0.0
return_profit = 0.0
...
# Per person calculation (dividing by total travelers)
per_person_selling = grand_total_selling / total_travelers_count if total_travelers_count > 0 else 0.0
per_person_sar = per_person_selling / sar_to_pkr if sar_to_pkr > 0 else 0.0
...
👥 *Total Travelers:* {total_travelers_count} Persons (Adults: {adults}, Children: {children}, Infants: {infants})
...
👉 *PKR {grand_total_selling:,.2f}* (Approx. SAR {grand_total_sar:,.2f})
👉 *Per Person Rate:* PKR {per_person_selling:,.2f} (~ SAR {per_person_sar:,.2f})
──────────────────────
_Thanks for choosing TravelNext. Aao Madiney Chalen!_"""
    ...
    <div class="invoice-box">
        ...
        <table class="details-table">
            ...
            <tr>
                <td><b>Total Travelers:</b> {total_travelers_count} Persons (Adults: {adults}, Child: {children}, Infant: {infants})</td>
        ...
        <div class="total-box">
            ...
            <p style="margin: 6px 0 0 0; font-size: 15px; color: #1b5e3a; font-weight: bold;">Per Person Rate: PKR {per_person_selling:,.2f} (~ SAR {per_person_sar:,.2f})</p>
            <p style="margin: 6px 0 0 0; font-size: 13.5px; color: #555;">Equivalent to approx. SAR {grand_total_sar:,.2f}</p>
        </div>
...
if st.button("Show Internal Profit Sheet 📈", key="btn_profit_sheet"):
    ...
    - **Arrival Flight Total ({total_travelers_count} Persons):** PKR {arrival_sell_total:,.2f} *(Profit: PKR {arrival_sell_total - arrival_orig_total:,.2f})*
