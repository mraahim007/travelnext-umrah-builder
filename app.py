...
def get_hotel_list(city):
    ...
    elif city == "Yay Madinah Apt":
        return [
            "-- Select Specific Apartment --",
            "Yay Madinah Apartment - Standard 2 Bed",
            "Yay Madinah Apartment - Deluxe 3 Grade",
            "Yay Madinah Apartment - Family Suite"
        ]
    return ["-- Select Specific Hotel --"]

# Helper to shorten city names for transport route titles
...
# Leg 1 Transport (Dynamic Routing Title)
...
if (city_1 != "-- Select City / Stop --") and (is_airport_stop_1 or nights_1 > 0):
    ...
    # Leg 2 Transport (Dynamic Routing Title from City 1 to City 2)
...
if (city_2 != "-- Select City / Stop --") and (("Airport" in city_2) or nights_2 > 0):
    ...
    if city_3 != "-- Select City / Stop --":
        # Leg 3 Transport (Dynamic Routing Title from City 2 to City 3)
...
# 7. OFFICIAL CUSTOMER QUOTATION SHEET WITH BUILT-IN PRINT / SAVE BUTTON & WHATSAPP TEXT
st.subheader("📄 Step 7: Official Customer Quotation & Sheet Preview")
...
✈️ *Airline:* {cust_airline if cust_airline != '-- Select Airline --' else 'N/A'}
...
🏨 *Itinerary & Services:*"""
    ...
    if city_1 != "-- Select City / Stop --":
        ...
        if not is_ap1:
            itinerary_html_rows += f"<tr><td><b>Leg 1</b></td><td><b>{city_1}</b></td><td>{hotel_1}<br>{nights_1} Nights, {num_rooms_1} Room(s) ({bed_type_1}), Meal: {meal_plan_1}<br>🚗 <i>Transfer:</i> {t_route_desc_1}</td></tr>"
            share_text += f"\n\n▫️ *Leg 1 ({city_1}):*\n   • Hotel: {hotel_1}\n   • Stay: {nights_1} Nights | {num_rooms_1} Room(s)\n   • Bed/View: {bed_type_1} | Meal: {meal_plan_1}\n   • Transfer: {t_route_desc_1}"
        else:
            itinerary_html_rows += f"<tr><td><b>Leg 1</b></td><td><b>{city_1}</b></td><td>Airport Transit / Stop<br>🚗 <i>Transfer:</i> {t_route_desc_1}</td></tr>"
    ...
    if city_2 != "-- Select City / Stop --":
        ...
        if not is_ap2:
            itinerary_html_rows += f"<tr><td><b>Leg 2</b></td><td><b>{city_2}</b></td><td>{hotel_2}<br>{nights_2} Nights, {num_rooms_2} Room(s) ({bed_type_2}), Meal: {meal_plan_2}<br>🚗 <i>Transfer:</i> {t_route_desc_2}</td></tr>"
            share_text += f"\n\n▫️ *Leg 2 ({city_2}):*\n   • Hotel: {hotel_2}\n   • Stay: {nights_2} Nights | {num_rooms_2} Room(s)\n   • Bed/View: {bed_type_2} | Meal: {meal_plan_2}\n   • Transfer: {t_route_desc_2}"
        else:
            itinerary_html_rows += f"<tr><td><b>Leg 2</b></td><td><b>{city_2}</b></td><td>Airport Transit / Stop<br>🚗 <i>Transfer:</i> {t_route_desc_2}</td></tr>"
            share_text += f"\n\n▫️ *Leg 2 ({city_2}):*\n   • Airport Transit / Stop\n   • Transfer: {t_route_desc_2}"
    ...
    if city_3 != "-- Select City / Stop --":
        ...
        if not is_ap3:
            itinerary_html_rows += f"<tr><td><b>Leg 3</b></td><td><b>{city_3}</b></td><td>{hotel_3}<br>{nights_3} Nights, {num_rooms_3} Room(s) ({bed_type_3}), Meal: {meal_plan_3}<br>🚗 <i>Transfer:</i> {t_route_desc_3}</td></tr>"
            share_text += f"\n\n▫️ *Leg 3 ({city_3}):*\n   • Hotel: {hotel_3}\n   • Stay: {nights_3} Nights | {num_rooms_3} Room(s)\n   • Bed/View: {bed_type_3} | Meal: {meal_plan_3}\n   • Transfer: {t_route_desc_3}"
        else:
            itinerary_html_rows += f"<tr><td><b>Leg 3</b></td><td><b>{city_3}</b></td><td>Airport Transit / Stop<br>🚗 <i>Transfer:</i> {t_route_desc_3}</td></tr>"
            share_text += f"\n\n▫️ *Leg 3 ({city_3}):*\n   • Airport Transit / Stop\n   • Transfer: {t_route_desc_3}"
...
_Thanks for choosing TravelNext. Aao Madiney Chalen!_"""
    ...
    # PRINTABLE INVOICE HTML CARD DESIGN WITH A BUILT-IN PRINT/SAVE PDF BUTTON
    invoice_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <div class="invoice-box">
        ...
        <table style="width: 100%; font-size: 13px; border-collapse: collapse; margin-bottom: 15px;">
            <tr style="background-color: #f2f9f5; text-align: left;">
                ...
                <th style="padding: 8px; border: 1px solid #ddd;">Details & Route Transfers</th>
            </tr>
            {itinerary_html_rows}
            <tr>
    ...
    components.html(invoice_html, height=580, scrolling=True)
    st.write("")

    encoded_text = urllib.parse.quote(share_text)
    whatsapp_url = f"https://wa.me/?text={encoded_text}"
    
    st.markdown(f'<a href="{whatsapp_url}" target="_blank"><button style="background-color:#25D366; color:white; padding:12px 20px; border:none; border-radius:8px; font-size:16px; font-weight:bold; cursor:pointer; width: 100%;">📤 Share Formatted Text via WhatsApp</button></a>', unsafe_allow_html=True)
    st.write("---")
