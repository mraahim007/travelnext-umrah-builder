...
if "num_legs" not in st.session_state:
    st.session_state.num_legs = 2  # Default 2 legs (Leg 1 & Leg 2)
...
# ================= STEP 5: HOTEL STAYS & UNLIMITED DYNAMIC LEGS =================
    ...
    for i in range(num_legs):
        ...
        with col_s1:
            ...
            # Date calculation helper for quotation
...
# ================= STEP 6: ZIARATS MANAGEMENT =================
    ...
    with col13:
        ...
        if ziarat_status == "YES":
            ...
            st.date_input("📅 Select Ziarat Date:", value=st.session_state.get('trip_start', date.today()), key="z_date")
    with col14:
        if ziarat_status == "YES":
            st.selectbox("Ziarat Transport Type:", vehicle_list, key="z_trans_type")
            st.radio("Currency (Ziarats):", ["PKR", "SAR"], horizontal=True, key="z_curr")
    with col15:
        if ziarat_status == "YES":
            z_curr_val = st.session_state.get("z_curr", "PKR")
            if z_curr_val == "PKR":
                parse_val(st.text_input("Ziarats Original Cost (PKR):", value="", key="z_orig_pkr"))
                parse_val(st.text_input("Ziarats Selling Price (PKR):", value="", key="z_sell_pkr"))
            else:
                parse_val(st.text_input("Ziarats Original Cost (SAR):", value="", key="z_orig_sar"))
                parse_val(st.text_input("Ziarats Selling Price (SAR):", value="", key="z_sell_sar"))
    st.markdown("</div>", unsafe_allow_html=True)
...
# ================= STEP 8: FINAL PROFESSIONAL QUOTATION PREVIEW =================
    ...
    if st.session_state.get('z_status') == 'YES':
        z_curr = st.session_state.get('z_curr', 'PKR')
        if z_curr == 'PKR':
            ziarat_sell = st.session_state.get('z_sell_pkr', 0.0)
        else:
            ziarat_sell = st.session_state.get('z_sell_sar', 0.0) * sar_to_pkr
        z_scope = st.session_state.get('z_scope', 'N/A')
        z_date = st.session_state.get('z_date', date.today())
        ziarat_row_html = f"""
        <tr style="border-bottom: 1px solid #e2e8f0;">
            <td style="padding: 10px; line-height: 1.6;"><b>Ziarats Included ({z_scope})</b><br><span style="color: #555; font-size: 12px;">Scheduled Date: {z_date.strftime('%d %b %Y')} | Vehicle: {st.session_state.get('z_trans_type', 'N/A')}</span></td>
            <td style="padding: 10px; text-align: right; line-height: 1.6;"><b>PKR {ziarat_sell:,.2f}</b></td>
        </tr>
        """
    ...
    if va_curr == 'PKR':
        va_sell_pp = st.session_state.get('va_sell_pp_pkr', 0.0)
    else:
        va_sell_pp = st.session_state.get('va_sell_sar_pp', 0.0) * sar_to_pkr
    ...
    st.success("✅ Professional Quotation compiled successfully with itemized line spacing!")
