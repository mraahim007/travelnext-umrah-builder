...
# Page Configuration with Dark Teal Theme Styling
st.set_page_config(page_title="TravelNext Umrah Builder", page_icon="🕋", layout="wide")

# Custom CSS for Dark Teal / Deep Blue Theme Styling
st.markdown("""
    <style>
    .stApp {
...
if current_step == 1:
    ...
    st.markdown("<h1 style='text-align: center; color: #41C9E2;'>✨ TRAVELNEXT INTERNATIONAL ✨</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: #f0f4f8;'>Smart Umrah Package Builder & Quotation System</h3>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #a0aec0; font-size: 18px;'>Aao Madiney Chalen 🕋 | Professional Booking & Management Portal</p>", unsafe_allow_html=True)
    st.write("---")
    st.info("💡 **Welcome!** This system helps you create professional, fast, and error-free Umrah packages. Please click the button below to proceed to the next step.")
...
# ================= STEP 7: VISA & AIRFARE =================
    ...
    with col19:
        if va_curr == "PKR":
            ...
            visa_sell_sar_pp = st.number_input("Selling Price Per Person (SAR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="va_sell_sar_pp")
    st.markdown("</div>", unsafe_allow_html=True)
...
# ================= STEP 8: FINAL QUOTATION PREVIEW =================
    ...
    invoice_html = f"""
    <div style="background-color: #ffffff; color: #222222; padding: 25px; border-radius: 10px; border: 2px solid #008DDA; font-family: Arial, sans-serif;">
        <h2 style="color: #008DDA; text-align: center; margin: 0;">TRAVELNEXT INTERNATIONAL</h2>
        ...
        <div style="background-color: #f2f9f5; padding: 15px; border-radius: 8px; text-align: center; border: 1px dashed #008DDA; margin-top: 15px;">
            <h3 style="color: #0b192c; margin: 0;">TOTAL PACKAGE INVESTMENT: PKR {grand_total_selling:,.2f}</h3>
    ...
    with col_act1:
        if st.button("📥 Save / Download PDF Report"):
            st.info("💡 PDF generation feature is active. (Use browser print -> Save as PDF for immediate hardcopy output).")
    with col_act2:
...
# ================= STEP 9: AGENCY PROFIT DASHBOARD =================
    ...
    """)
    st.balloons()
    st.markdown("</div>", unsafe_app:=True)
