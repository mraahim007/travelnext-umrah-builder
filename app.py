...
# 7. OFFICIAL CUSTOMER QUOTATION (Clean Streamlit Elements UI)
...
if st.button("Generate Official Quotation Form 📋", type="primary", key="btn_gen_quotation"):
    ...
    # Official Header Box
    st.markdown("---")
    st.markdown("<h2 style='text-align: center; color: #2e8b57;'>✨ TRAVELNEXT INTERNATIONAL ✨</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #aaaaaa;'>Official Umrah Packages & Travel Services</p>", unsafe_allow_html=True)
    st.write("---")
    
    # Client Summary Columns
    q_col1, q_col2 = st.columns(2)
    with q_col1:
        st.markdown(f"**Client Name:** {cust_first_name or 'Valued'} {cust_last_name or 'Customer'}")
        st.markdown(f"**Total Travelers:** {total_people} Persons ({adults} Adults, {children} Children)")
        st.markdown(f"**Total Duration:** {total_stay_nights} Nights")
    with q_col2:
        st.markdown(f"**Travel Date:** {trip_start_date.strftime('%d %b %Y')}")
        st.markdown(f"**Airline / Flight:** {cust_airline if cust_airline != '-- Select Airline --' else 'N/A'}")
        st.markdown(f"**Landing Airport:** {cust_landing if cust_landing != '-- Select Airport --' else 'N/A'}")
    ...
    if city_1 != "-- Select City --":
        ...
        st.markdown(f"* **Leg 1 ({city_1}):** {'Jeddah Transit / Stop' if is_j1 else hotel_1}")
        if not is_j1:
            st.markdown(f"  * *Duration:* {nights_1} Nights ({d1.strftime('%d %b')} - {d2.strftime('%d %b %Y')}) | *Transport:* {trans_1}")
        else:
            st.markdown(f"  * *Transport:* {trans_1}")

    # Leg 2 Display
    if city_2 != "-- Select City --":
        is_j2 = "Jeddah" in city_2
        st.markdown(f"* **Leg 2 ({city_2}):** {'Jeddah Transit / Stop' if is_j2 else hotel_2}")
        if not is_j2:
            st.markdown(f"  * *Duration:* {nights_2} Nights ({d2.strftime('%d %b')} - {d3.strftime('%d %b %Y')}) | *Transport:* {trans_2}")
        else:
            st.markdown(f"  * *Transport:* {trans_2}")

    # Leg 3 Display
    if city_3 != "-- Select City --":
        is_j3 = "Jeddah" in city_3
        d4 = d3 + timedelta(days=nights_3)
        st.markdown(f"* **Leg 3 ({city_3}):** {'Jeddah Transit / Stop' if is_j3 else hotel_3}")
        if not is_j3:
            st.markdown(f"  * *Duration:* {nights_3} Nights ({d3.strftime('%d %b')} - {d4.strftime('%d %b %Y')}) | *Transport:* {trans_3}")
        else:
            st.markdown(f"  * *Transport:* {trans_3}")

    st.write("---")
    st.markdown("### 🗺️ Ziarats & Services")
    if ziarat_status == "YES":
        st.markdown(f"- **Ziarats Included:** {ziarat_scope} (Transport: {ziarat_transport_type})")
    else:
        st.markdown("- **Ziarats:** Not Included")
    ...
    st.markdown(
        f"""
        <div style="background-color: #111111; padding: 20px; border-radius: 10px; border: 2px solid #2e8b57; text-align: center;">
            <h3 style="color: #2e8b57; margin: 0;">TOTAL PACKAGE INVESTMENT</h3>
            <h1 style="color: #ffffff; margin: 10px 0;">PKR {grand_total_selling:,.2f}</h1>
            <h3 style="color: #2e8b57; margin: 0;">(~ SAR {grand_total_sar:,.2f})</h3>
        </div>
        """,
        unsafe_allow_html=True
    )
    
    st.markdown("<p style='text-align: center; font-style: italic; color: #aaaaaa; margin-top: 25px;'>\"THANKS FOR CHOOSING TRAVELNEXT. AAO MADINEY CHALEN! MAY ALLAH ACCEPT YOUR GOOD DEEDS!\" 🕋✨</p>", unsafe_allow_html=True)
    st.markdown("---")
