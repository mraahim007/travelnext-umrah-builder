...
st.markdown("Configure your stay legs. Enter values either in **PKR** or **SAR** (Both will automatically sync):")
...
with col_s3:
    curr_type_1 = st.radio("Currency Input Type (Leg 1 Hotel):", ["PKR", "SAR"], horizontal=True, key="curr_type_1")
    if not is_jeddah_1:
        if curr_type_1 == "PKR":
            orig_pn_1 = st.number_input("Original Cost Per Night 1 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="c1_orig_pn_pkr")
            sell_pn_1 = st.number_input("Selling Price Per Night 1 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="c1_sell_pn_pkr")
            orig_per_night_1 = orig_pn_1
            sell_per_night_1 = sell_pn_1
        else:
            orig_pn_sar_1 = st.number_input("Original Cost Per Night 1 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="c1_orig_pn_sar")
            sell_pn_sar_1 = st.number_input("Selling Price Per Night 1 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="c1_sell_pn_sar")
...
with col_t3:
    if t_curr_1 == "PKR":
        trans_orig_1 = st.number_input("Transport Org Cost 1 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="t1_orig_pkr")
        trans_sell_1 = st.number_input("Transport Sell Cost 1 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="t1_sell_pkr")
    else:
        t_orig_sar_1 = st.number_input("Transport Org Cost 1 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="t1_orig_sar")
        t_sell_sar_1 = st.number_input("Transport Sell Cost 1 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="t1_sell_sar")
...
if (city_1 != "-- Select City --") and (is_jeddah_1 or nights_1 > 0):
    ...
    with col_s5:
        if not is_jeddah_2:
            ...
            st.info("ℹ️ Jeddah selected: No hotel stay required.")
    with col_s6:
        curr_type_2 = st.radio("Currency Input Type (Leg 2 Hotel):", ["PKR", "SAR"], horizontal=True, key="curr_type_2")
        if not is_jeddah_2:
            if curr_type_2 == "PKR":
                orig_pn_2 = st.number_input("Original Cost Per Night 2 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="c2_orig_pn_pkr")
                sell_pn_2 = st.number_input("Selling Price Per Night 2 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="c2_sell_pn_pkr")
                orig_per_night_2 = orig_pn_2
                sell_per_night_2 = sell_pn_2
            else:
                orig_pn_sar_2 = st.number_input("Original Cost Per Night 2 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="c2_orig_pn_sar")
                sell_pn_sar_2 = st.number_input("Selling Price Per Night 2 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="c2_sell_pn_sar")
    ...
    with col_t6:
        if t_curr_2 == "PKR":
            trans_orig_2 = st.number_input("Transport Org Cost 2 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="t2_orig_pkr")
            trans_sell_2 = st.number_input("Transport Sell Cost 2 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="t2_sell_pkr")
        else:
            t_orig_sar_2 = st.number_input("Transport Org Cost 2 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="t2_orig_sar")
            t_sell_sar_2 = st.number_input("Transport Sell Cost 2 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="t2_sell_sar")
...
if (city_2 != "-- Select City --") and (("Jeddah" in city_2) or nights_2 > 0):
    ...
    with col_s9:
        if city_3 != "-- Select City --":
            curr_type_3 = st.radio("Currency Input Type (Leg 3 Hotel):", ["PKR", "SAR"], horizontal=True, key="curr_type_3")
            if not is_jeddah_3:
                if curr_type_3 == "PKR":
                    orig_pn_3 = st.number_input("Original Cost Per Night 3 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="c3_orig_pn_pkr")
                    sell_pn_3 = st.number_input("Selling Price Per Night 3 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="c3_sell_pn_pkr")
                    orig_per_night_3 = orig_pn_3
                    sell_per_night_3 = sell_pn_3
                else:
                    orig_pn_sar_3 = st.number_input("Original Cost Per Night 3 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="c3_orig_pn_sar")
                    sell_pn_sar_3 = st.number_input("Selling Price Per Night 3 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="c3_sell_pn_sar")
    ...
    if city_3 != "-- Select City --":
        ...
        with col_t9:
            if t_curr_3 == "PKR":
                trans_orig_3 = st.number_input("Transport Org Cost 3 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="t3_orig_pkr")
                trans_sell_3 = st.number_input("Transport Sell Cost 3 (PKR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="t3_sell_pkr")
            else:
                t_orig_sar_3 = st.number_input("Transport Org Cost 3 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="t3_orig_sar")
                t_sell_sar_3 = st.number_input("Transport Sell Cost 3 (SAR):", min_value=0.0, value=0.0, step=50.0, format="%.2f", key="t3_sell_sar")
...
with col15:
    if z_curr == "PKR":
        ziarat_orig_cost = st.number_input("Ziarats Original Cost (PKR):", min_value=0.0, value=0.0, step=1000.0, format="%.2f", key="z_orig_pkr")
        ziarat_sell_cost = st.number_input("Ziarats Selling Price (PKR):", min_value=0.0, value=0.0, step=1000.0, format="%.2f", key="z_sell_pkr")
    else:
        z_orig_sar = st.number_input("Ziarats Original Cost (SAR):", min_value=0.0, value=0.0, step=100.0, format="%.2f", key="z_orig_sar")
        z_sell_sar = st.number_input("Ziarats Selling Price (SAR):", min_value=0.0, value=0.0, step=100.0, format="%.2f", key="z_sell_sar")
...
with col18:
    if va_curr == "PKR":
        visa_air_orig_pp = st.number_input("Original Cost Per Person (PKR):", min_value=0.0, value=0.0, step=5000.0, format="%.2f", key="va_orig_pp_pkr")
    else:
        va_orig_sar_pp = st.number_input("Original Cost Per Person (SAR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="va_orig_pp_sar")
        visa_air_orig_pp = va_orig_sar_pp * sar_to_pkr
with col19:
    if va_curr == "PKR":
        visa_air_sell_pp = st.number_input("Selling Price Per Person (PKR):", min_value=0.0, value=0.0, step=5000.0, format="%.2f", key="va_sell_pp_pkr")
    else:
        va_sell_sar_pp = st.number_input("Selling Price Per Person (SAR):", min_value=0.0, value=0.0, step=500.0, format="%.2f", key="va_sell_pp_sar")
        visa_air_sell_pp = va_sell_sar_pp * sar_to_pkr
...
# 7. OFFICIAL CUSTOMER QUOTATION
st.subheader("📄 Step 7: Official Customer Quotation")
if st.button("Generate Quotation 📋", type="primary", key="btn_gen_quotation"):
    ...
    st.markdown("### ✨ TRAVELNEXT INTERNATIONAL ✨")
    st.markdown("*Official Umrah Packages & Travel Services | Aao Madiney Chalen*")
    st.write("---")
    
    col_q1, col_q2 = st.columns(2)
    with col_q1:
        st.write(f"**Client Name:** {cust_first_name or 'Valued'} {cust_last_name or 'Customer'}")
        st.write(f"**Total Travelers:** {total_people} Persons ({adults} Adults, {children} Children)")
        st.write(f"**Total Duration:** {total_stay_nights} Nights")
    with col_q2:
        st.write(f"**Travel Date:** {trip_start_date.strftime('%d %b %Y')}")
        st.write(f"**Airline / Flight:** {cust_airline if cust_airline != '-- Select Airline --' else 'N/A'}")
        st.write(f"**Landing Airport:** {cust_landing if cust_landing != '-- Select Airport --' else 'N/A'}")
    
    st.write("---")
    st.markdown("#### 🏨 Itinerary & Accommodation Breakdown")
    
    if city_1 != "-- Select City --":
        is_j1 = "Jeddah" in city_1
        if not is_j1:
            st.write(f"- **Leg 1 ({city_1}):** {hotel_1} | {nights_1} Nights ({d1.strftime('%d %b')} - {d2.strftime('%d %b %Y')}) | Transport: {trans_1}")
        else:
            st.write(f"- **Leg 1 ({city_1}):** Jeddah Transit / Stop | Transport: {trans_1}")

    if city_2 != "-- Select City --":
        is_j2 = "Jeddah" in city_2
        if not is_j2:
            st.write(f"- **Leg 2 ({city_2}):** {hotel_2} | {nights_2} Nights ({d2.strftime('%d %b')} - {d3.strftime('%d %b %Y')}) | Transport: {trans_2}")
        else:
            st.write(f"- **Leg 2 ({city_2}):** Jeddah Transit / Stop | Transport: {trans_2}")

    if city_3 != "-- Select City --":
        is_j3 = "Jeddah" in city_3
        d4 = d3 + timedelta(days=nights_3)
        if not is_j3:
            st.write(f"- **Leg 3 ({city_3}):** {hotel_3} | {nights_3} Nights ({d3.strftime('%d %b')} - {d4.strftime('%d %b %Y')}) | Transport: {trans_3}")
        else:
            st.write(f"- **Leg 3 ({city_3}):** Jeddah Transit / Stop | Transport: {trans_3}")

    ziarat_text = f"{ziarat_scope} (Transport: {ziarat_transport_type})" if ziarat_status == "YES" else "Not Included"
    st.write(f"- **Ziarats:** {ziarat_text}")
    
    st.write("---")
    st.info(f"### 💰 TOTAL PACKAGE INVESTMENT: PKR {grand_total_selling:,.2f}  |  SAR {grand_total_sar:,.2f}")
    st.markdown("*Thanks for choosing TravelNext. Aao Madiney Chalen! May Allah accept your good deeds! 🕋✨*")
    st.write("---")
