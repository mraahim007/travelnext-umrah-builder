...
# Initialize legs data up to 10 safely without pre-filling forcing 0s
...
# ================= STEP 8: FINAL PROFESSIONAL QUOTATION PREVIEW =================
    ...
    for i in range(num_legs):
        ...
        if not is_apt:
            ...
            if curr_h == 'PKR':
                h_sell_pn = float(st.session_state.get(f'leg_sell_pn_pkr_{i}', 0.0))
            else:
                h_sell_pn = float(st.session_state.get(f'leg_sell_pn_sar_{i}', 0.0)) * sar_to_pkr
        ...
        t_sell = 0.0
        if t_curr == 'PKR':
            t_sell = float(st.session_state.get(f'leg_t_sell_pkr_{i}', 0.0))
        else:
            t_sell = float(st.session_state.get(f'leg_t_sell_sar_{i}', 0.0)) * sar_to_pkr
    ...
    if st.session_state.z_status == 'YES':
        ...
        if z_scope in ["MAKKAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            m_sell = float(st.session_state.z_makkah_sell_pkr) if m_curr == 'PKR' else float(st.session_state.z_makkah_sell_sar) * sar_to_pkr
            ziarat_sell += m_sell
        ...
        if z_scope in ["MADINAH ONLY", "BOTH MAKKAH AND MADINAH"]:
            ...
            md_sell = float(st.session_state.z_madinah_sell_pkr) if md_curr == 'PKR' else float(st.session_state.z_madinah_sell_sar) * sar_to_pkr
            ziarat_sell += md_sell
    ...
    if va_curr == 'PKR':
        va_sell_pp = float(st.session_state.va_sell_pp_pkr)
    else:
        va_sell_pp = float(st.session_state.va_sell_pp_sar) * sar_to_pkr
...
# ================= STEP 9: AGENCY PROFIT DASHBOARD =================
    ...
    for i in range(st.session_state.num_legs):
        ...
        if curr_h == 'PKR':
            c_cost = float(st.session_state.get(f'leg_cost_pn_pkr_{i}', 0.0))
            c_sell = float(st.session_state.get(f'leg_sell_pn_pkr_{i}', 0.0))
        else:
            c_cost = float(st.session_state.get(f'leg_cost_pn_sar_{i}', 0.0)) * sar_to_pkr
            c_sell = float(st.session_state.get(f'leg_sell_pn_sar_{i}', 0.0)) * sar_to_pkr
        ...
        t_curr = st.session_state.get(f'leg_t_curr_{i}', 'PKR')
        if t_curr == 'PKR':
            t_cost = float(st.session_state.get(f'leg_t_cost_pkr_{i}', 0.0))
            t_sell = float(st.session_state.get(f'leg_t_sell_pkr_{i}', 0.0))
        else:
            t_cost = float(st.session_state.get(f'leg_t_cost_sar_{i}', 0.0)) * sar_to_pkr
            t_sell = float(st.session_state.get(f'leg_t_sell_sar_{i}', 0.0)) * sar_to_pkr
